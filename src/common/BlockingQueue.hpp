...
template <typename T>
void BlockingQueue<T>::push_impl(T value)
{
    {
...
template <typename T>
void BlockingQueue<T>::close_impl() noexcept
{
    {
        std::scoped_lock lock{mutex_};
        closed_ = true;
...
template <typename T>
bool BlockingQueue<T>::is_closed_impl() const noexcept
{
    std::scoped_lock lock{mutex_};
    return closed_;
}
...
template <typename T>
bool BlockingQueue<T>::empty_impl() const noexcept
{
    std::scoped_lock lock{mutex_};
    return queue_.empty();
}
...
template <typename T>
std::size_t BlockingQueue<T>::size_impl() const noexcept
{
    std::scoped_lock lock{mutex_};
    return queue_.size();
}
